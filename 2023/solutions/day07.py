...
class Day07(Day):
    ...
    def count_cards(self, hand: str) -> dict:
        hand_dict = {}
        for card in hand:
    ...
    def get_hand_score(self, hand: str) -> int:
        """
        Hand order:
        ...
        hand_dict = self.count_cards(hand)
    ...
    def get_adjusted_hand_score(self, hand: str) -> int:
        """
        Hand order:
    ...
    def get_hand_order(self, hand: str) -> int:
        output_str = ""
        for card in hand:
            card_value = Day07.ORDER.find(card)
            output_str += ('0' if len(str(card_value)) == 1 else '') + str(card_value)
        return int(output_str)

    def get_adjusted_hand_order(self, hand: str) -> int:
        output_str = ""
        for card in hand:
            card_value = Day07.ADJUSTED_ORDER.find(card) + 1
            output_str += ('0' if len(str(card_value)) == 1 else '') + str(card_value)
        return int(output_str)
    ...
    def part_one(self, raw_data: str):
        ...
        for hand, bid in hands:
            data.append((hand, bid, self.get_hand_score(hand), self.get_hand_order(hand)))
    ...
    def part_two(self, raw_data: str):
        ...
        for hand, bid in hands:
            data.append((hand, bid, self.get_adjusted_hand_score(hand), self.get_adjusted_hand_order(hand)))
